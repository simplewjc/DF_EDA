"""
Based on https://github.com/sshaoshuai/MTR/blob/master/tools/test.py
"""
...
from eda.models import model as model_utils
...
def parse_config():
    ...
    parser.add_argument('--tcp_url', type=str, default='tcp://127.0.0.1', help='tcp url for distrbuted training')
    parser.add_argument('--tcp_port', type=int, default=18888, help='tcp port for distrbuted training')
    parser.add_argument('--local_rank', type=int, default=0, help='local rank for distributed training')
    parser.add_argument('--fix_random_seed', action='store_true', default=False, help='')
    parser.add_argument('--set', dest='set_cfgs', default=None, nargs=argparse.REMAINDER,
                        help='set extra config keys if needed')
    parser.add_argument('--nms_thresh', type=float, default=None, help='NMS distance in meter(m)')
    ...
    if args.nms_thresh is not None:
        cfg.MODEL.MOTION_DECODER.NMS_DIST_THRESH = args.nms_thresh

    return args, cfg


def pred_with_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False):
    # load checkpoint
...
def main():
    ...
    if args.launcher == 'none':
        ...
        total_gpus, cfg.LOCAL_RANK = getattr(common_utils, 'init_dist_%s' % args.launcher)(
            args.tcp_url, args.tcp_port, args.local_rank, backend='nccl'
        )
        dist_test = True
    ...
    if not args.eval_all:
        ...
        if 'best' in args.ckpt:
            epoch_id = 'best'
        elif 'latest' in args.ckpt:
            epoch_id = 'latest'
    ...
    with torch.no_grad():
        if args.eval_all:
            ...
            pred_with_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test)
