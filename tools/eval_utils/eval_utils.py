"""
Based on https://github.com/sshaoshuai/MTR/blob/master/tools/eval_utils/eval_utils.py
"""
...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, logger_iter_interval=50):
    ...
    thresh = cfg.MODEL.MOTION_DECODER.NMS_DIST_THRESH
    if thresh >= 0:
        result_file = f'result_nms-{thresh}.pkl'
    else:
...
def pred_result(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, logger_iter_interval=50):
    result_dir.mkdir(parents=True, exist_ok=True)
    ...
    thresh = cfg.MODEL.MOTION_DECODER.NMS_DIST_THRESH
    test_name = cfg.DATA_CONFIG.SPLIT_DIR.test
    if thresh >= 0:
